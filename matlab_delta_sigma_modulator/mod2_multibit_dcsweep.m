% Title: simplified 2nd-order multi-bit incremental modulator with CIFB topology
...
% different reconstruction filters for a multi-bit 2nd-order modulator, including CoI2, CoI3, sinc2, sinc3, and the proposed L2min2, L2min2s filters
...
clear;
format long;
OSR             = 250;          %oversampling ratio of the converter
...
for i = 1:N
    ...
    for j=1:OSR
        ...
        if (j==1)
            ...
            q(i,j) = min(max(round(Y(1,j)/LSB),-(q_level-1)/2),(q_level-1)/2);  %multibit dac
        end
    end
...
parfor i = 1:N
    temp = q_linear(i,:);
    %coi2 filter
    vo_coi2(i,1) = sum(temp.*(OSR:-1:1))/sum(1:1:OSR)*LSB;
    %coi3 filter
    coefficient_tmp = conv(1:1:OSR,ones(1,OSR));
    coefficient_coi3 = flip(coefficient_tmp(1:OSR));
    vo_coi3(i,1) = sum(temp.*coefficient_coi3)/sum(coefficient_coi3(1:OSR))*LSB;  
    %sinc2 filter
    ...
    %l2min2 filter
    vo_l2min2(i,1) = sum(temp.*coefficient_l2min2)/sum(coefficient_l2min2(1:OSR))*LSB;
    %l2min2s filter
    vo_l2min2s(i,1)= sum(temp(1:length(coefficient_l2min2s)).*coefficient_l2min2s)/sum(coefficient_l2min2s)*LSB;
end
