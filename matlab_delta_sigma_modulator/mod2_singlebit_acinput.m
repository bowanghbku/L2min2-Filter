...
wn = zeros(1,tmpN);
for i=1:tmpN
    wn(i) = tmpN*(tmpN+1)/2 - sum(0:1:i-1);
end
wn = [flip(wn),wn(2:end)];
...
for itotal  =  1:N/OSR               %total number of incremental conversion points
    ...
    for i = 1:OSR
        ...
        if (i==1)
            ...
            U2(1,i) = 0;
            Y(1,i) = vinsp(i+(itotal-1)*OSR)*a3+c1*U1(1,i)+U2(1,i);
            q(i,i) = 0;
        else
            U1(1,i) = U1(1,i-1)+a1*vinsp(i+(itotal-1)*OSR)-b1*q(1,i-1) + a1*noise_in;
            U2(1,i) = U2(1,i-1)+U1(1,i-1)+a2*vinsp(i)-b2*q(1,i-1)+a2*noise_in;
            Y(1,i) = a3*vinsp(i)+c1*U1(1,i)+U2(1,i)+a3*noise_in;
            if Y(1,i) >= 0
                q(1,i) = 1;
            else
                q(1,i) = -1;
    ...
    deci_out_coi2(1,itotal)     = sum(q.*[OSR:-1:1])/sum([OSR:-1:1]);                %output reconstruction
    deci_out_coi3(1,itotal)     = sum(q(1:length(coefficient_coi3)).*coefficient_coi3)/sum(coefficient_coi3);                 
    deci_out_sinc2(1,itotal)    = sum(q(1:length(coefficient_sinc2)).*coefficient_sinc2)/sum(coefficient_sinc2);            
    deci_out_sinc3(1,itotal)    = sum(q(1:length(coefficient_sinc3)).*coefficient_sinc3)/sum(coefficient_sinc3);     
...
set(gca,'YTick',-200:40:0,'FontName','Times New Roman','FontSize',12);
xlim([0 0.5]);
ylim([-200 0]);
